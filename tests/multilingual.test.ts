...
async function main() {
  ...
  assert('1 默认中文 Tab 含记录', tabZh.includes('记录'), `tabText=${tabZh.replace(/\s+/g, ' ').slice(0, 60)}`);
  ...
  assert('2 英文 Tab 含 Records', tabEn.includes('Records'), `tabText=${tabEn.replace(/\s+/g, ' ').slice(0, 60)}`);
  assert('3 英文 Tab 不含记录', !tabEn.includes('记录'), `含记录=${tabEn.includes('记录')}`);
  ...
  assert('4 切回中文 Tab 含记录', tabZh2.includes('记录'), `tabText=${tabZh2.replace(/\s+/g, ' ').slice(0, 60)}`);
