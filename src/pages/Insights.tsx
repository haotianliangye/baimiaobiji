...
import { PieChart, Loader2, Sparkles, ChevronLeft, ChevronRight, Calendar, AlertCircle, ChevronDown, ChevronUp, Trash2, Copy, RefreshCw, MessageCircle, Save, Edit2, Lightbulb } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import ReactMarkdown from 'react-markdown';
import ContextChat from '../components/ContextChat';
import { db, Insight } from '../db/db';
import { useSettingsStore } from '../store/settings.store';
import { format, subDays, startOfMonth, endOfMonth, startOfWeek, endOfWeek, eachDayOfInterval, addMonths, isSameMonth, parseISO, isToday } from 'date-fns';
...
// ——— Inline date-picker popover (no native <input type="date">) ———
...
function DatePickerPopover({
  ...
  return (
    <div ref={containerRef} className="relative">
      {/* Trigger button — mimics the native date input appearance */}
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
      ...
      {open && (
        <div
          className={`absolute top-full mt-1.5 z-50 bg-white border border-stone-200 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.12)] p-3 w-[240px] animate-in fade-in zoom-in-95 duration-150 ${
