...
interface AppState {
  ...
  generateReview: (id: string, dateStr: string, logs: any[], diaryContent: string, targetPromptIndex?: number) => Promise<void>;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  generateReview: async (id, dateStr, logs, diaryContent, targetPromptIndex) => {
    if (!logs || logs.length === 0) return;
    ...
    // 如果指定了 targetPromptIndex，优先采用；如果未指定但该日记本来就有 review_prompt_index，沿用已有的；否则用 settings 的全局默认值
    let activePromptIndex = targetPromptIndex !== undefined 
      ? targetPromptIndex 
    ...
    if (targetPromptIndex === undefined) {
      ...
      if (existing && existing.review_prompt_index !== undefined) {
        activePromptIndex = existing.review_prompt_index;
      }
    }
