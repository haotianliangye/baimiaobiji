...
export default function Layout() {
  ...
  const {
    ...
    checkAndGenerateScheduledInsights,
  ...
  // #C1 v0.6.0 洞察周报/月报自动生成调度。
  // 触发条件（两个 settings 开关独立控制）：
  // - 周报：insightAutoGenWeeklyEnabled && now >= 本周一 00:01 && lastInsightWeeklyRun < 本周一 00:01
  // - 月报：insightAutoGenMonthlyEnabled && now >= 本月 1 号 00:01 && lastInsightMonthlyRun < 本月 1 号 00:01
  // 时间范围：上周 / 上月；按 mingwuInsightSelectedIndices 拆 N 个 task 入队（每个 slot 一个）。
  // 静默跳过：该周期内无 raw_logs 且无 thoughts，或 db.insights 已存在同周期记录。
  useEffect(() => {
    checkAndGenerateScheduledInsights();
  }, []);
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0eef5] text-stone-900 overflow-hidden items-center justify-center">
      ...
      >
        ...
        {/* #C1 v0.6.0 洞察自动生成通知 Toast（mysteria 渐变区别于 review/diary 深色） */}
        {autoGeneratedInsightNotification && (
          <div className="bg-gradient-to-r from-baimiao-mysteria to-[#2c2957] text-white text-[13px] px-4 py-2.5 flex items-center justify-between shrink-0 animate-in slide-in-from-top duration-300 z-40 border-b border-white/10">
            <span className="flex items-center gap-1.5 font-medium select-none text-[12.5px]">
              <span className="animate-pulse">🔮</span>
              {t('notify.autoInsight', {
                rangeLabel: autoGeneratedInsightNotification.rangeLabel,
                count: autoGeneratedInsightNotification.count,
              })}
            </span>
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-white text-baimiao-mysteria px-2.5 py-1 rounded-md text-[11px] font-semibold hover:bg-white/90 transition-colors"
              >
                {t('notify.view')}
              </button>
              <button
                onClick={clearAutoGeneratedInsightNotification}
