...
export default function CalendarHeatmap({ currentDate, onSelectDate, onClose }: CalendarHeatmapProps) {
  ...
  const getIntensityClass = (count: number, isSelected: boolean) => {
    ...
    if (count < 3) return 'bg-[#c6e4d1] rounded-[3px]';
    if (count < 8) return 'bg-[#8ad6a3] rounded-[3px]';
    if (count < 15) return 'bg-[#50c27b] rounded-[3px]';
    return 'bg-[#2ea043] rounded-[3px]'; // standard github green
  };
      ...
      >
        ...
        <div className="p-6 pt-2">
           <button 
             ...
             className="w-full bg-[#1bc46c] text-white py-3.5 rounded-xl text-[14px] font-medium tracking-wide shadow-sm hover:bg-[#19b362] transition-colors active:scale-[0.98] flex items-center justify-center gap-2"
           >
