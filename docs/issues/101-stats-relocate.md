...
2. 作为用户，我希望碎屑页底部输入框左上方显示"今日 X 条 X 字"，以便一眼看到当日输入量。
3. 作为用户，我希望回顾页卡片列表底部显示"今日 X 条 X 字"，以便看到当日回顾量。
4. 作为用户，我希望沉思页输入框左上方显示"今日 X 条 X 字"，以便看到当日沉思量。
...
- seam: `foundation-migration.test.ts`（跨页统计小字：碎屑/回顾/沉思/明悟）
- seam: `thoughts.test.ts`（沉思页统计小字）
...
- 测试重点（PRD Testing 第 1 条）：四页统计显示位置/口径/明悟不显示/0 条仍显示。即：
  ...
  - 碎屑页底部输入框左上方、回顾页卡片列表底部、沉思页输入框左上方显示"今日 X 条 X 字"。
  - 明悟页不显示任何"今日"统计。
  - 各页统计口径正确（碎屑 raw_logs / 回顾 ai_editorial||ai_review / 沉思 thoughts 按 created_at）。
