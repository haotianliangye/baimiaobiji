...
| # | 需求对齐原文 | 状态 | 说明 |
|---|---|---|---|
| G1 | #3+#4：「沉淀页副标题为'今日 X 字'；中间区域为**瀑布流/时间线下拉式胶囊切换器**」「两个选项居中合并成一个胶囊切换器」「下拉式胶囊：默认显示当前选中项（首次默认'瀑布流'），点击后展开两个选项；选择后收起复位为单个胶囊；切换后立即刷新内容。」 | missing | PRD Problem Statement 提到了"沉淀页存在独立的'瀑布流/时间线'切换区"这一**问题**，但 Solution 12 条和 9 个 seam 中无任何一条包含下拉胶囊切换器的解决方案。Issue 006 (Seam 5) 更在 Out of Scope 中明确写"不改动瀑布流/时间线视图切换机制"，主动排除了此需求。**全链封死。** |
...
| G10 | #1：「点击 +N 覆盖层：进入该条拾微的详情/完整视图（页面或底部模态层），**可查看全部附件、完整摘要、原始内容**。」 | partial | PRD Seam 4 写"点击 +N 进入该记录的详情页/完整视图"，但遗漏了详情视图中应包含"全部附件、完整摘要、原始内容"三部分内容的要求。 |
...
|---|---|---|---|
| Testing Decisions > 测试类型与工具：「组件/单元测试：使用项目现有测试框架（Vitest + React Testing Library，若已配置）」「端到端测试（可选但推荐）：使用 Playwright/Cypress」「视觉回归测试：建议对顶部栏、设置页、多媒体卡片、沉淀卡片、附件面板做快照对比」 | 所有 issue | missing | 9 个 issue 的 Testing Decisions 均只写"验证..."验收点，无一提及具体测试框架（Vitest+RTL）、E2E 工具（Playwright/Cypress）或视觉回归测试方法。PRD 的测试工具选型决策在 issue 层全部丢失。 |
...
| Further Notes > 性能：「沉淀卡片折叠态不应预渲染完整 Markdown，可使用 line-clamp 或最大高度截断」 | 006 (Seam 5) | missing | Issue 006 Implementation Decisions 写了折叠行数和渐变遮罩，但完全未提"不预渲染完整 Markdown"的性能约束和实现建议（line-clamp/最大高度截断）。 |
...
### 1. 沉淀页瀑布流/时间线下拉式胶囊切换器（#3 + #4 -> PRD -> issue 全链缺失）
...
PRD Further Notes 明确列了三条无障碍要求（aria-label、屏幕阅读器、焦点状态），分别应进 Seam 1 / Seam 5 / Seam 6 三个 issue，但三个 issue 均未收录。作为跨 seam 的全局约束，丢失后将影响顶部栏按钮、沉淀折叠/展开、附件面板三个关键交互区域的可访问性。
...
- G1 沉淀瀑布流/时间线切换：Thoughts.tsx 已有切换机制，但形式是否为"下拉胶囊"待确认
...
- G7 沉淀展开不限高：Thoughts.tsx 展开态是否限高，待确认
