# Progress - Seam 5: 沉淀卡片限高折叠与缩略图规范 (#006)

## 改动摘要

为沉淀页（Thoughts）卡片引入折叠态/展开态与统一的多媒体缩略图规范，解决长文卡片无限延伸、缩略图比例与数量不统一的问题。
...
- `src/i18n/zh.ts` - 新增 4 条沉淀卡片折叠/缩略图 key
- `src/i18n/en.ts` - 新增 4 条沉淀卡片折叠/缩略图 key
...
3. **video/audio 缩略图为图标占位**：未抽取视频首帧（需加载 `<video>` 元素，开销较大），用 `Film`/`Music` 图标占位。当前 RichEditor 仅支持图片附件（`accept="image/*"`），实际沉淀数据中 video/audio 附件暂不会出现，占位为前瞻性兼容。
