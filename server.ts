...
async function startServer() {
  ...
  // Batch 1: async + assertSafeBaseUrl 校验 → 拒绝内网/loopback/metadata
  // 防止 `settings.baseUrl = 'http://169.254.169.254/'` 之类的 SSRF。
...
4. 使用 Markdown 格式输出，用中文回答。`;
  ...
  app.post('/api/transcribe', express.json({limit: '50mb'}), async (req, res) => {
    try {
       ...
       if (provider === 'gemini') {
         ...
         const safeBase = await assertSafeBaseUrl(baseUrl || 'https://ark.cn-beijing.volces.com/api/v3');
         const baseStr = safeBase.replace(/\/chat\/completions$/, '').replace(/\/responses$/, '').replace(/\/$/, '');
         const apiUrl = `${baseStr}/responses`;
         ...
         // Batch 1: assertSafeBaseUrl 拒内网/metadata host
         const safeBase = await assertSafeBaseUrl(baseUrl || defBase);
         const baseStr = safeBase;
  ...
  async function callMinimaxTTS(
    ...
    try {
      ...
      // MiniMax t2a_v2 默认返回 hex 编码 mp3；保留 base64 回退以兼容旧/异常返回
      try {
