...
### 🎨 Superhuman 轻奢级视觉重构与像素级细节优化
全站 UI 进行了一次高品质的视觉调和与动画细节升级，达成精致统一的“纸质社论”与“暮光紫”轻奢质感：
- **品牌 Logo 重绘**：顶部 Logo 采用 `font-serif` 衬线宋体字家族，字重降为 `font-normal`（正常），且垂直向微调下移 2 像素（`translate-y-[2px]`），实现与右侧动作图标的绝对垂直中线平齐。
- **无抖动发光卡片**：重新设计了时光碎屑自适应气泡卡片，Hover 时去除了物理位移，改为纯粹的淡紫色微光外发光漫射。通过引入子像素抗锯齿保护与 3D 渲染图层阻断，彻底解决了合成层切换时小字号文本虚化、抖动和变色的浏览器渲染缺陷。
- **暮光紫录音态**：录音激活状态条变更为优雅的暮光紫渐变破晓色（`from-baimiao-mysteria to-[#2c2957]`），且时间戳加深，消除了原有黑条块的不协调感。
- **设置面板整体品质升级**：将系统设置页背景底色统一为高档的纯白色调，重绘了 Tab 选项卡边框。设置页面下的 8 处大功能模块卡片全部套用 `.baimiao-card-diary`，支持紫光悬浮微弱上浮；底部主按键配套重绘为暮光紫渐变主按钮。
